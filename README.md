# Devskiller programming task sample - Blockchain
...
This repo contains a sample project for Blockchain and below you can
...
Any **JavaScript** project can be used as a Blockchain programming task. 
...
The "test" script definition, which is located in the [package.json](package.json) file:

```sh
truffle migrate --reset --network develop && truffle test --network develop
```

It is required to enable the "Ethereum" external component when uploading the task in order to run Ganache at `ethereum:8545`.

Candidates can test their solution using the local instance:
```sh
...
```json
{
  "verification": {
    ...
    "pathPatterns": ["**test/*.verification.test.js"]
  },
  "readOnlyFiles" : [ "package.json", "truffle.js", "test/**" ]
}
```
